...
case $1 in
	...
	for dist in centos amazon rocky almalinux; do
		...
		for arch in aarch64 x86_64; do
		    ...
		    mkdir -p $ARTIFACTS_DIR/lts/5/$dist_dest/$release/$arch
		    find $REPOSITORY_PATH/$dist/$release/$arch -name '*.rpm' -not -name '*debug*' -exec cp {} $ARTIFACTS_DIR/lts/5/$dist_dest/$release/$arch \;
		done
	    done
	done
	;;
