...
<xsl:stylesheet
    ...
    <xsl:key
        name="conf-search"
        match="wix:Component[wix:File/@Source = '$(var.ProjectSourceDir)\etc\fluentd\fluentd.conf']"
        use="@Id" />
    <xsl:template match="*[self::wix:Component or self::wix:ComponentRef][key('conf-search', @Id)]" />
    <xsl:key
        name="service-bat-search"
        match="wix:Component[wix:File/@Source = '$(var.ProjectSourceDir)\bin\fluentd.bat']"
        use="@Id" />
