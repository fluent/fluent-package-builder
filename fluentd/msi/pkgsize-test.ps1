...
try {
    ...
    $previous_msi_name = "fluentd-${previous_version}-x64.msi"
    $response = Invoke-WebRequest -Uri "${base_uri}/4/windows/${previous_msi_name}" -OutFile $previous_msi_name -PassThru
}
catch {
    $heroku_uri = 'http://fluentd-package-browser.herokuapp.com'
...
$msi = (Get-Item "fluentd\\msi\\repositories\\fluentd-*.msi") | Sort-Object -Descending { $_.LastWriteTime } | Select-Object -First 1
